...
def create_instrumented_tool(tool: Tool, audit_client: AuditClient) -> Tool:
    ...
    def instrumented_func(*args, **kwargs):
        ...
        try:
            ...
            event = AuditEvent(
                ...
                latency_ms=latency_ms,
                metadata={"tool_name": tool.name, "error": str(e)},
            )
            audit_client.capture(event)
    ...
    return Tool(name=tool.name, description=tool.description, func=instrumented_func)
...
def create_demo_agent(audit_client: AuditClient):
    ...
    search_tool = Tool(
        name="web_search",
        description="Search the web for current information. Use for recent info or facts.",
        func=search.run,
    )
    ...
    file_tool = Tool(
        name="read_file", description="Read the contents of a file. Input should be a filename.", func=read_file
    )
...
def main():
    ...
    with AuditClient(api_url="http://localhost:8000") as audit_client:
        ...
        print()
        print("🌐 Open UI:")
        print("   Open demo/ui/index.html in your browser")

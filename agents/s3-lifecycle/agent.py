...
class S3LifecycleAgent:
    ...
    def investigate(self) -> dict:
        ...
        if list_decision.allowed:
            ...
            try:
                ...
                demo_buckets = [
                    ...
                    if fnmatch.fnmatch(b['Name'], 'aimgentix-demo-*')
                ]
        ...
        if risk_level != "high":
            ...
            create_decision = self._check_policy(
                "s3:CreateBucket",
            ...
            if create_decision.allowed:
                proposed_action = {
                    ...
                    "policy_decision": {
                        "allowed": True,
                        "requires_approval": create_decision.requires_approval
                    }
                }
                ...
                if create_decision.requires_approval:
                    ...
                    findings.append({
                        "type": "policy_note",
                        "message": "Action requires approval per policy",
                        "severity": "info"
                    })
            else:
    ...
    def act(self) -> dict:
        ...
        try:
            ...
            results["policy_decisions"].append({
                "action": "unknown",
                "allowed": False,
                "reason": pv.decision.reason
            })
            ...
            try:
                ...
                if 'Contents' in objects:
                    for obj in objects['Contents']:
                        print(f"   📜 Checking policy for s3:DeleteObject...")
                        try:
                            ...
                            policy_decisions.append({"action": "s3:DeleteObject", "allowed": False, "reason": pv.decision.reason})
                            errors.append({"bucket": bucket_name, "key": obj['Key'], "error": f"Policy violation: {pv.decision.reason}"})
                            print(f"   ❌ Policy violation: {pv.decision.reason}")
                            continue

                # Delete bucket - CHECK POLICY FIRST
                print(f"   📜 Checking policy for s3:DeleteBucket...")
